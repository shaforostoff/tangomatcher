/* **************************************************************************
  This file was part of Lokalize
...
static void prepareLists(QString str, QStringList& main, QStringList& space, const QString& accel=QString(), QString markup=QString())
{
    Q_UNUSED(accel);
    int pos=0;
                            ...
                            int options)
{
    ...
    prepareLists(str1ForMatching, s1, s1Space);
    prepareLists(str2ForMatching, s2, s2Space);
...
//     kWarning()<<"wordDiff 1 '" <<result<<"'";
    ...
    if (options&Html)
    {
        res.replace("{KBABELADD}","<font style=\"background-color:#99CCFF;color:black\">");
        res.replace("{/KBABELADD}","</font>");
        res.replace("{KBABELDEL}","<font style=\"background-color:#FF9999;color:black\">");
